...
function redraw() {
	if(dirty_grid != 0) {
		...
		if(mode == 0) {
			for(i1=0;i1<8;i1++) {
				for(i2=min[i1];i2<=max[i1];i2++)
					leds[i1*8 + i2] = L0;
				leds[i1*8 + count[i1]] = L1;
				if(position[i1] >= 0) {
					leds[i1*8 + position[i1]] = L2;
				}
			}
		}
		// SHOW SPEED
			...
			for(i1=0;i1<8;i1++) {
				if(position[i1] >= 0)
					leds[i1*8 + position[i1]] = L0;

				leds[i1*8 + speed[i1]+8] = L1;
		...
		// SHOW RULES
			...
			leds[edit_row * 8] = L1;
			leds[edit_row * 8 + 1] = L1;

			leds[rule_dests[edit_row] * 8 + 5] = L2;
			leds[rule_dests[edit_row] * 8 + 6] = L2;

			for(i1=8;i1<8;i1++)
				leds[rules[edit_row] * 8 + i1] = L0;
		...
		if(vb==1) {
			...
			for(y=0;y<YSIZE;y++) {
				for(x=0;x<8;x++) {
					if(leds[x+y*8]>3) mask0 = mask0 | 1<<x; // if on at all, set to full bright
					else mask0 = mask0 & ~(1<<x); // if off, leave cell off
				}
				for(x=0;x<8;x++) {
					if(leds[x+y*8]>3) mask1 = mask1 | 1<<x; // if on at all, set to full bright
